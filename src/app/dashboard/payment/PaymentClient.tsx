...
export default function PaymentClient() {
  ...
  const fetchSubscription = async () => {
    try {
      ...
      setAmount(!isNaN(remaining) && remaining > 0 ? remaining.toString() : sub.plan_price);
      setLoading(false);
    } catch (e) {
  ...
  const fetchPendingSubscription = async () => {
    try {
      ...
      const pendingSub: Subscription | undefined = res.data.find(
        (s: Subscription) => s.status === "PENDING"
      );
      if (pendingSub) {
        setSubscription(pendingSub);
        const remaining = Number(pendingSub.amount_due);
        setAmount(!isNaN(remaining) && remaining > 0 ? remaining.toString() : pendingSub.plan_price);
      }
      setLoading(false);
    } catch (e) {
          ...
          <form onSubmit={handleSubmit} className="p-8">
            ...
            <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-xl p-4 mb-6">
              <p className="text-sm text-yellow-800 dark:text-yellow-200">
                ⚠️ <strong>Paiement manuel:</strong> Enregistrez le montant ci-dessous pour validation par
                l&apos;administrateur.
              </p>
            </div>
